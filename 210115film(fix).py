...
def main():
    ...
    for i in range(len(names)):
        sql="insert into test3(id,name,value,date,area,genre,quote) values("+str(numlist[i])+","+"'"+names[i]+"'"+","+str(values[i])+","+str(re.findall('\d{4}',others[i][0])[0])+","+'"'+others[i][1]+'"'+","+'"'+others[i][2]+'"'+","+'"'+quotes[i]+'"'+")"
        #test为表名，由于list[0]和list[1]的元素类型是整型，所以需要将其转化为字符串，list[2]本身为字符串类型，但需要加入单引号''，
